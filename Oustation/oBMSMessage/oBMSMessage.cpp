...
char *oBMSMesFormat(oBMSMessage mes) {
    ...
    json_object_set_number(root_object, "Value", mes.val);
...
double oBMSMesVal(char *mes_string){
    ...
    valString =  json_object_get_string(objjson,"Value");
    double valFloat = atof(valString);
    Serial.printf("the val as a string: %s\n",valString);
    Serial.printf("the val as a float: %f\n",valFloat);
